...
def _generate_version_py() -> str:
    """Generate app/_version.py from git tags using dunamai."""
    version = dunamai.Version.from_git()
    distance = version.distance if version.distance is not None else 0
    parts = [version.base]
    if distance > 0:
        parts.extend([f"+{distance}", f"+{version.commit}"])
    if version.dirty:
        parts.append(".dirty")
    serialized = "".join(parts)
    version_path = BASE_DIR / "app" / "_version.py"
    version_path.write_text(f'# Auto-generated by build.py - do not edit manually\nVERSION = "{serialized}"\n')
    click.echo(f"Generated {version_path} with version {serialized}")
    return serialized
    ...
    if dist_dir.exists():
        ...
        if platform == "Linux":
            ...
            with tarfile.open(archive_path, "w:gz") as tar:
                tar.add(dist_dir, arcname="EchoView")
        else:
            archive_path = build_dir / f"EchoView-{version}-win64.zip"
            with zipfile.ZipFile(archive_path, "w", zipfile.ZIP_DEFLATED) as zf:
                for file in dist_dir.rglob("*"):
                    if file.is_file():
                        zf.write(file, Path("EchoView") / file.relative_to(dist_dir))
